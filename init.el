...
(use-package org
  ...
  (setq org-capture-templates
	`(("i" "ideas" entry (file+olp"~/Projects/Notes/agenda/idea.org" "idea lists")
	   "** %?\n \n %a")
	  ("f" "Fleeting notes" entry (file+olp"~/Projects/Notes/agenda/inbox.org" "Captures")
	   "** TODO %?\n \n %a")
	  ))
...
(setq org-roam-node-display-template "${tags:50} ${title:*}")

;; Config the org-roam-ui for better visuilization. 
(use-package org-roam-ui
  ...
  (setq org-roam-ui-sync-theme t
        org-roam-ui-follow t       ;; 图谱跟随 Emacs 中打开的节点
        org-roam-ui-update-on-save t
        org-roam-ui-open-on-start t))

(setq minibuffer-visible-completions nil)
;; Better search performance when the nodes it too much.
(use-package consult-org-roam
  :ensure t
  :after org-roam
  :init
  (require 'consult-org-roam)
  ;; 开启 consult-org-roam 模式
  (consult-org-roam-mode 1)
  :bind
  ("C-c n b" . consult-org-roam-backlinks) ;; 查看当前笔记的反向链接
  :config
  ;; Turn off the preview of consult.
...
(use-package org-download
  ...
  ;; Set the default directory to store images.
  (setq-default org-download-image-dir "~/Projects/Notes/notes/images") 
...
(setq org-roam-capture-templates
   '(
     ;; Project template, create when start a new project things.
     ("p" "project" plain "%?"
     :if-new (file+head "projects/%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n#+filetags: :Project:")
     :unnarrowed t)
     
     ;; Literature node, create when learning something and do not form my own thoughs.
     ("l" "literature" plain "%?"
     :if-new (file+head "reference/%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n#+filetags: :Literature:")
     :unnarrowed t)

     ;; Zettel(permanent) node, create from fleeting(闪念) and literature nodes. Which is
     ;; my own thoughs.
     ("z" "zettel(permanent)" plain "%?"
